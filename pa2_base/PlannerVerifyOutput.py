...
input_file_ls = ['data/mdp/continuing-mdp-10-5.txt', 'data/mdp/continuing-mdp-2-2.txt', 'data/mdp/continuing-mdp-50-20.txt',
                 'data/mdp/episodic-mdp-10-5.txt', 'data/mdp/episodic-mdp-2-2.txt', 'data/mdp/episodic-mdp-50-20.txt']
...
class VerifyOutputPlanner:
    def __init__(self, algorithm, print_error):
        ...
        for algo in algorithm_ls:
            ...
            for in_file in input_file_ls:
                ...
                print('test case', str(counter), algo,
                      ":\t", " ".join(cmd_planner))
                counter += 1
    ...
    def verifyOutput(self, cmd_output, in_file, pe):
        ...
        sol_file = in_file.replace(
            "continuing", "sol-continuing").replace("episodic", "sol-episodic")
        ...
        if not len(est) == nstates:
            mistakeFlag = True
            print("\n", "*"*10, "Mistake:Exact number of line in standard output should be",
                  nstates, "but have", len(est), "*"*10)
        ...
        if pe not in pe_ls:
            ...
            for i in range(len(est)):
                ...
                print("%10.6f" % est_V, "%10.6f" %
                      base_V, "%10.6f" % abs(est_V-base_V), end="\t")
