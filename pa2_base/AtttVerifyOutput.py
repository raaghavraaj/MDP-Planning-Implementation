...
def run(states, policy, player):
    ...
    cmd_decoder = "python", "decoder.py", "--value-policy", "verify_attt_planner", "--states", states, "--player-id", str(
        player)
...
def verifyOutput(states, output, player):
    ...
    if output[0] != player:
        print("\n", "*"*10,
              f"Mistake: First line of policy file should be the player id, i.e.'{player}'")
        sys.exit()
    output.remove('')
    ...
    if len(output)-1 != len(states):
        print("\n", "*"*10,
              f"Mistake: Expected {len(states)} policy lines, got {len(output)-1}")
        sys.exit()
    ...
    for idx, out in enumerate(output[1:]):
        ...
        try:
            ...
            print("\n", "*"*10,
                  f"Mistake: In line {idx+2}, Number format excpetion. {out}")
            sys.exit()

    states_intersection = set(states).intersection(set(policy_states))
    if len(states_intersection) != len(states):
        print("\n", "*"*10,
              f"Mistake: States in policy file and input states file do not match")
        sys.exit()
...
if __name__ == '__main__':
    parser.add_argument("--states", required=True, type=str,
                        help="File with valid states of the player")
    parser.add_argument("--policy", required=True, type=str,
                        help="Policy file of the opponent player")
